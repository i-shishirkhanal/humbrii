...
const PaymentSuccess = () => {
  ...
  useEffect(() => {
    const verifyPayment = async () => {
      ...
      try {
        const data = await verifyEsewaPayment(transactionData);
            ...
            <div className="bg-muted/50 rounded-lg p-4 mb-8 text-left">
              ...
              <div className="space-y-1 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-green-600 capitalize">{booking.booking_status}</span>
                </div>
                {booking.paid_amount && (
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Amount Paid</span>
                    <span>NPR {booking.paid_amount.toLocaleString()}</span>
                  </div>
                )}
              </div>
            </div>
